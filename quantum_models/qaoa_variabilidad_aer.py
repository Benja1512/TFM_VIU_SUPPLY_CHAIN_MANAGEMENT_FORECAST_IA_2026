import os
import pandas as pd
import matplotlib.pyplot as plt
from qiskit_optimization import QuadraticProgram
from qiskit_algorithms import QAOA
from qiskit_algorithms.optimizers import COBYLA
from qiskit.primitives import Sampler
from qiskit.providers.aer import AerSimulator
from qiskit.primitives.backend_sampler import BackendSampler
from qiskit_optimization.algorithms import MinimumEigenOptimizer

# ===============================
# BACKEND RUIDOSO (AerSimulator)
# ===============================
backend = AerSimulator()
sampler = BackendSampler(backend)

# ===============================
# CONFIGURACIÓN
# ===============================
num_corridas = 10
results_dir = "quantum_models/results"
csv_path = os.path.join(results_dir, "qaoa_multi_output_aer.csv")
img_path = os.path.join(results_dir, "graficos/qaoa_variabilidad_aer.png")
os.makedirs(os.path.dirname(img_path), exist_ok=True)

# ===============================
# DEFINICIÓN DEL PROBLEMA
# ===============================
def crear_problema():
    qp = QuadraticProgram()
    qp.binary_var("x_AT1")
    qp.binary_var("x_AT2")
    qp.binary_var("x_BT1")
    qp.binary_var("x_BT2")
    qp.linear_constraint({"x_AT1": 1, "x_AT2": 1}, "<=", 2, "capacidad_A")
    qp.linear_constraint({"x_BT1": 1, "x_BT2": 1}, "<=", 2, "capacidad_B")
    qp.linear_constraint({"x_AT1": 1, "x_BT1": 1}, ">=", 2, "demanda_T1")
    qp.linear_constraint({"x_AT2": 1, "x_BT2": 1}, ">=", 2, "demanda_T2")
    qp.minimize(linear={"x_AT1": 2, "x_AT2": 3, "x_BT1": 4, "x_BT2": 1})
    return qp

# ===============================
# EJECUTAR QAOA VARIAS VECES
# ===============================
resultados = []
for i in range(1, num_corridas + 1):
    print(f"🔁 Corrida {i}")
    qp = crear_problema()
    qaoa = QAOA(optimizer=COBYLA(maxiter=200), reps=2, sampler=sampler)
    qaoa_optimizer = MinimumEigenOptimizer(qaoa)
    result = qaoa_optimizer.solve(qp)

    # Mostrar resultados
    print("   ➤ Costo:", result.fval)
    print(f"   ➤ x_AT1 = {result.x[0]} | x_AT2 = {result.x[1]} | x_BT1 = {result.x[2]} | x_BT2 = {result.x[3]}")
    print("-" * 50)

    resultados.append({
        "corrida": i,
        "costo": result.fval,
        "x_AT1": result.x[0],
        "x_AT2": result.x[1],
        "x_BT1": result.x[2],
        "x_BT2": result.x[3]
    })

# ===============================
# GUARDAR CSV
# ===============================
df = pd.DataFrame(resultados)
df.to_csv(csv_path, index=False)
print(f"\n✅ Resultados guardados en {csv_path}")

# ===============================
# GRAFICAR
# ===============================
plt.figure(figsize=(8, 4))
plt.bar(df["corrida"], df["costo"], color='darkorange', edgecolor='black')
plt.xlabel("Corrida QAOA")
plt.ylabel("Costo total")
plt.title("Variabilidad con AerSimulator (QAOA)")
plt.grid(axis='y', linestyle='--', alpha=0.6)
for i, val in enumerate(df["costo"]):
    plt.text(i + 1, val + 0.3, f"{val:.1f}", ha='center')
plt.tight_layout()
plt.savefig(img_path)
plt.show()
print(f"📊 Gráfico guardado en {img_path}")
